...
const Page = () => {
    ...
    const [subTitleKey, setSubTitleKey] = useState<number>(0);
    const [isEditTitle, setIsEditTitle] = useState<boolean>(true);
    ...
    const selAddSubtitleHandler = async () => {
        ...
        if( book.bookId > 0){
            ...
            console.log(typeof response.data.data.id);
        ...
        if(addSubTitle !== '' && book.bookId > 0){
            ...
            if(minKey !== null && minKey > 0) {
                try{
                    ...
                    dispatch(edit({subtitleList: new Map([[response.data.id, response.data.subtitle]])}));
    ...
    const subtitleHandler = (key : number) => {
        console.log(key);
        setSubTitleKey(key);
        dispatch(keyStore({currentSubtitle: key}));
    }
                    ...
                    <div className="w-60 m-auto pt-7">
                        <div
                            ...
                            <nav
                                ...
                                {Array.from(book.subtitleList, ([key, value]) => (
                                    value !== '' ?
                                        <div className="relative h-11 w-full min-w-[200px] flex justify-between"
                                             key={key}>
